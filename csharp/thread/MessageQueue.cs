...
using System;
using com.meyer.muscle.queue.Queue;
 ...
 */
namespace com.meyer.muscle.thread {
public class MessageQueue implements MessageListener, Runnable
{
   ...
   {
      public ListControlMessage(MessageListener l, bool add)
      {
         _listener = l;
         _add = add;
      }
   ...
   private LinkedList _messageQueue = new LinkedList();      // shared (synchronized this)
   private MessageListener _listener = null;       // used if we have exactly one listener
   private LinkedList _listeners = null;                // used if we have more than one listener
   private bool _threadActive = false;          // shared (synchronized this)
   private bool _interruptPending = false;      // shared (synchronized this)
   private System.Threading.Thread _myThread = null;                // set inside of run() (synchronized this)
